...
function collision(w){
  for(var i=0; i<w.length; i++){
    ...
    var boundsW = Matter.Bounds.create([{x:pos.x-w[i].w/2,y:pos.y-w[i].h/2},
                                       {x:pos.x+w[i].w/2,y:pos.y+w[i].h/2}]);
    var boundsB = Matter.Bounds.create([{x:ball.circle.position.x-ball.circle.circleRadius-3,y:ball.circle.position.y-ball.circle.circleRadius},
                                      {x:ball.circle.position.x+ball.circle.circleRadius+3,y:ball.circle.position.y+ball.circle.circleRadius+3}]);
    if(Matter.Bounds.overlaps(boundsW, boundsB)){return true}
  }
  return false;
}
...
function squareCollision(s){
  for(var i=0; i<squares.length; i++){
    ...
    var sw = s[i].w;
    var sh = s[i].h;
    var boundsS = Matter.Bounds.create([{x:sx-sw/2,
                                         y:sy-sh/2},
                                        {x:sx+sw/2,
                                         y:sy+sh/2}]);
    var boundsB = Matter.Bounds.create([{x:ball.circle.position.x-ball.circle.circleRadius-3,
...
function lavaCollision(l){
  for(var i=0; i<lavas.length; i++){
    if(l[i].touchable){
      ...
      var sw = l[i].w;
      var sh = l[i].h;
      var boundsL = Matter.Bounds.create([{x:sx-sw/2,
                                           y:sy-sh/2},
                                          {x:sx+sw/2,
                                           y:sy+sh/2}]);
      var boundsB = Matter.Bounds.create([{x:ball.circle.position.x-ball.circle.circleRadius-1,
      ...
      if(Matter.Bounds.overlaps(boundsL, boundsB)){
        ...
        ball.lives -=1;
        setTimeout(function() {
          Matter.Body.setPosition(ball.circle, {x: game.levelObject.orgx, y:game.levelObject.orgy})
          paused = false;
          return true;
        }, delay);
      }
...
function collisionDiamond(d){
  ...
  var x2 = d.x+10;
  var y2 = d.y+10;
  var boundsD = Matter.Bounds.create([{x:x,y:y},
                                     {x:x2,y:y2}]);
  var boundsB = Matter.Bounds.create([{x:ball.circle.position.x-ball.circle.circleRadius-3,y:ball.circle.position.y-ball.circle.circleRadius-3},
                                    {x:ball.circle.position.x+ball.circle.circleRadius+3,y:ball.circle.position.y+ball.circle.circleRadius+3}]);
